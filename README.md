...
```
*The text engine heavily weights the NLI cross-checking against trusted sources. If a user inputs a fake claim that contradicts live news feeds, the fake score spikes to 95%+.*
...
Because "Video" deepfakes can manipulate *just* the audio, *just* the visuals, or *both*, we process them concurrently.
...
# GNEWS_API_KEY, MEDIASTACK_API_KEY, etc.
```
...
3. *Crucial:* In Vercel Environment Variables, set `VITE_API_URL` to your Hugging Face Space URL. *(e.g., `https://yourusername-truthlens-backend.hf.space/api/v1/detect`)*
4. Hit Deploy! 
